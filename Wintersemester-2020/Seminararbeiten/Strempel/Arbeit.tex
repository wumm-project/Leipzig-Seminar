...
prototypical link
\begin{center}\tt
  OldSystem -- isTransformedInto $\to$ NewSystem
\end{center}
...
The listing of 25 aspects of evolutionary potential analysis in \cite[Fig.
  4.91]{KS} points to another abstraction phenomenon we encounter in the
course of these conceptualisations -- the \emph{transition from verb to noun},
from a process to a structure view. If (ibid.) one of the transformation rules
is formulated as a verb as «5. coordinate the actions», the corresponding
pattern is formulated as noun «5. coordinated actions». This seemingly small
difference has a major impact on RDF modelling, as it involves a transition
from the place of the predicate to the place of the subject or object. This is
not a super problem in the context of RDF language constructs, since
properties of predicates can also be expressed in RDF, but it is problematic
for visualisation as an RDF graph. There, edges would then have to become
additionally nodes. Since the aim of this work is to conceptualise the
structure of evolutionary trees in close relation to their graphical
representation, predicates are consistently used for this purpose, e.g.
\begin{center}
  \texttt{tc:tranfsformedBySeparationPattern} instead of
  \texttt{tc:SeparationPattern} 
\end{center}
even if the namespace \texttt{tc:} of the TRIZ concepts is extended to
predicates.
