...
\begin{document}
    ...
    \begin{center}
        \begin{tabular}{|c|c|}\hline
            ...
            ex:FlowModelAsRequired, tc:FlowTransformatio \\
    ...
    \texttt{tc:FlowAnalysis}, with \texttt{od:applyChanges} all approaches of
    the Flow Analysis are finally gathered in the \texttt{tc:FlowTransformation} 
    and can be applied to the real system (level 0).
    ...
    that prescribe certain proceedings.[source] \emph{They} are 
    ...
    of exhaust flow innovation. Given the context of tc:PollutionRestrictions, 
    we define the goal ex:SatisfactionOfPollutionRestrictions with the 
    requirements to reduce the exhaust flow while the work flow needs to be 
    preserved as good as possible (ex:InnovationOfExhaustFlow od:hasRequirement 
    ex:ReduceExhaustFlow, ex:PreserveWorkFlow). We define a specific
    ...
    (called ex:ICEExhaustionModel and ex:buildICEExhaustionInnovationModel).
    ...
    Using the tc:RulesofEvalution we can define an ex:EvaluationOfExhaustFlow, that
    detetcs a tc:FlowDefect which needs to be fixed, which can be found in the 
    ...
    Using these informations, certain tc:TechniquesOfModification can be applied to fix the
    tc:FlowDefect and fullfill the set tc:Requirements, 
    \begin{code}
    ...
    leading to an additional flow ex:FuelAdditive and upgraded channel component of the 
    ex:Exhaust flow,
    \begin{code}tt
    ex:FuelAdditive \\
    \> a tc:Flow ; \\
    ...
    resulting in the final \emph{ModelAsRequired} ex:buildICEExhaustionInnovationModel
    \begin{code}
    ex:ICEExhaustionInnovationModel \\
