...
\item Doppelpendel und chaotische Trajektorien, deterministisches Chaos
\item Magnetisches Pendel

  Pendel an einem festen Stab mit magnetischem Ende, das zwischen drei im
  Dreieck aufgestellten Magneten pendelt und dabei um jeweils einen der
  Magneten längere Zeit kreist, bis es chaotisch zu einem anderen Magneten
  wechselt.
\end{itemize}
...
\item Dreikörperproblem und das Kolmogorow-Arnold-Moser-Theorem
\end{itemize}
...
\begin{itemize}[noitemsep]
\item Grenzzyklen
\item Attraktor als stabile Lösung des entsprechenden
...
\item Der Lorenzattraktor.
  ...
  ist schwer zu unterscheiden, ob das chaotische Verhalten 
  \begin{itemize}[noitemsep]
  \item Ergebnis des Aufschaukelns numerischer Fehler in einer dynamischen
    Bahnverfolgung ist
  \item oder der Attraktor als \emph{globales} Artefakt bestimmt wurde.
  \end{itemize}
\item \emph{Seltsame Attraktoren} als „Endzustand eines dynamischen Prozesses,
  dessen fraktale Dimension nicht ganzzahlig und dessen Kolmogorov-Entropie
...
\item Beispiel: Doppelpendel als Pendel, aber der Pendelkörper hat selbst noch
  ...
  Das wurde in einem Handexperiment vorgeführt. Das Obersystem prägt dem
  Untersystem durch Energieeintrag eine gemeinsame Dynamik auf. Obwohl das
  Doppelpendel eigentlich chaotisch ist, ist das System damit (final)
  \emph{nicht} chaotisch, sondern verhält sich wie ein einfaches Pendel mit
...
\begin{itemize}[noitemsep]
\item immersiver und submersiver Systembegriff (dazu ein Einschub weiter
  unten)
\item Projektionsphänomene -- Entstehung von Singularitäten bei Reduktion: Die
...
Ziel dieses Termins war es u.a., eine Liste von Begrifflichkeiten
...
Lydie Laforet vorgestellt, was die gesamte Seminarzeit in Anspruch nahm.
...
\section{Anmerkungen zum Seminar am 05.11.2019}
...
\begin{enumerate}
\item eine neue argumentative Tradition des Verhältnisses von Induktion und
  Deduktion (einen Begriff von „Science“ in der im angelsächsischen Sprachraum
  verbreiteten Bedeutung) und
\item eine Aufspaltung in Einzelwissenschaften, deren Vertreter den intern
  stehenden (deduktiven) Ganzheitsanspruch an das jeweilige Theoriegebäude
  gern mit dem alten (induktiven) Ganzheitsanspruch einer „Welterklärung“
  verwechseln (Naturphilosophie, Empiriokritizismus).
\end{enumerate}
...
Fachlogiken, die T.S. Kuhn als \emph{Paradigmata} bezeichnet, stehen
ihrerseits aber in dialektischen Entwicklungsprozessen bis hin zu Brüchen
(Kuhn untersucht derartige Paradigmenwechsel intensiv).
...
liegt ein solcher mathematisch-deduktiver Geschlossenheitsanspruch zugrunde,
...
begründeteten Geschlossenheitsansprüche jeder Einzelwissenschaft nur zu einem
...
\section{Anmerkungen zum Seminar am 12.11.2019}
...
\emph{9-Felder-Matrix} oder bei den \emph{Gesetzen der Evolution technischer
  Systeme} auf, ist aber andererseits nur unscharf gegen Begriffe wie
Funktion, Komponente, Element, Produkt oder Objekt abgegrenzt. Genau um die
Problematik einer solchen Abgrenzung ging es in jener Diskussion und auch in
...
institutionalisierte „äußere“ Bedingungen (Supersystem in der
...
Das Begreifen einer derartigen \emph{begriffliche Weiterentwicklung} (in der
...
\section{ Anmerkungen zum Seminar am 19.11.2019}
...
\item Abgrenzung des Systems nach innen durch Zusammenfassen von Teilbereichen
  ...
  über Input/Output-Beziehungen reduziert wird.
...
Betrachtungen keine Rolle und werden auch im Ansatz von (Ulanowicz 2009) nicht
berücksichtigt, wenn allein \emph{Flüsse} T\_ij zwischen Systemkomponenten
betrachtet werden, nicht aber deren zeitliche Strukturierung. Eigenzeiten,
deren systemische Quelle noch zu identifizieren ist, spielen eine große Rolle
in der Taktung sowohl biologischer („biologische Uhr“) als auch technischer
(Taktfrequenzen) Systeme und damit im koordinierten Zusammenspiel solcher
Systeme.
...
(Mingers 1989) als sehr speziell erweist.
...
Zeitskalen der Eigenzeiten von Mikro- und Makrosystem weit auseinander
liegen. Dann kann auf kleinen Zeitskalen der durch das Makrosystem induzierte
...
\section{Anmerkungen zum Seminar am 26.11.2019}

Thema: Resilienz (IIRM)

Literatur: (Holling 2000), (Walker 2004),

Zusatzliteratur: (Brand 2007) 
...
\textbf{drei Reduktionsdimensionen} als „Reduktion auf das Wesentliche“
thematisiert: 
\begin{itemize}
...
\item[2.] Abgrenzung des Systems nach innen durch Zusammenfassen von
  ...
  „Verhaltenssteuerung“ über Input/Output-Beziehungen reduziert wird.
...
Betrachtung eines Systems als „System von Systemen“ nahe, wie es in (Holling
2001) thematisiert wird. Wesentliches Reduktionskriterium für Beziehungen (3)
sind in solchen Systemen spezifische Eigenzeiten und Eigenräume wie in den
...
werden. Dazu heißt es in (Holling 2001:392) „semi-autonomous levels are formed
from the interactions among a set of variables that share similar speed (and,
we would add, geometric/spatial attributes)“. Hinzuzufügen aus einer früheren
...
Derartige Verhältnisse werden in anderen Kontexten als \textbf{synergetisch}
...
Teile ist) ergibt sich aus dem \emph{dynamischen Prozess} der Wechselwirkung
der Systemkomponenten, wie er in den Abbildungen 4 und 5 in (Holling 2001)
...
Betracht bleibt. Die Systembeschreibung ist damit eine \textbf{Fiktion} im
Sinne des Gebrauchs dieses Begriffs in der Vorlesung.
...
Systementwicklung“, die dort als \textbf{adaptiver Zyklus} bezeichnet wird.
...
Gründen mit \emph{Störungen} (disturbances) aufgeladen ist. Bilder wie die
...
Bedeutungen des Resilienzbegriffs“ wird in (Brand 2007) klar
...
Theorie der {K-Wellen} (Kondratjew-Zyklus) bekannt und spielen auch in
...
Reduktion lassen sich Ebenen von Systemen wie bei (Holling 2011) sinnvoll
...
auch als \emph{Versklavungseffekt} bezeichnet. Dies wird allerdings durch die
...
qualitative) \emph{Fähigkeit} eines Systems, seine
...
vom aktuellen Systemzustand wird. Eine Quantifizierung von Widerständigkeit in
einem solchen Verständnis ist insbesondere nach einer Reorganisation des
...
(Holling 2001) eine Rolle spielt, als \emph{Panarchie} -- in (Brand 2007) auch
als \emph{Metasystem} bezeichnet - und reserviere die Bezeichnung
\emph{System} für die bisher eingenommene Abstraktionsebene, in der „adaptiver
Zyklus“ für die Beschreibung der Dynamik dieses Systems auf einer hohen
...
resilience“ genannt: 
\begin{itemize}
\item Latitude: Wie weit sind kritische Punkte (threshold) voneinander
  entfernt?
\item Resistance: Wie „widerständig“ ist ein System gegen Veränderung, wie
  „schnell“ bewegt es sich im Phasenraum?
\item Precariousness: Wie nahe ist der Systemzustand am nächsten kritischen
  Punkt?
\item Panarchy: Welche „cross-scale interactions“ haben Einfluss auf das
  System?
\end{itemize}
...
\begin{itemize}[noitemsep]
\item Volker Stollorz. Elinor Ostrom und die Wiederentdeckung der Allmende.
\item Silke Helfrich, Felix Stein. Was sind Gemeingüter?
\item Leonard Dobusch, Sigrid Quack. Auf dem Weg zu einer Wissensallmende?
\end{itemize}
...
treten. Diese drei begrifflichen Ansätze nehmen dabei ihrerseits Reduktionen
...
Formierung eines Konzepts von \emph{Resilienz} als auch zu den Konzepten, die
in (Ashby 1958) und (Boisot 2011) für eine Systembewertung auf der Basis rein
...
\emph{System von Parametern} zur Bewertung sozial-ökologischer Systeme
vorgeschlagen.
...
\item Im 4. Fall ist nicht klar, warum die $y$-Achse fehlt und von Symbiose
  geredet wird.
\item Die begriffliche Verwendung von Transition und Transformation in ihrer
...
allerdings zu explizieren -- etwas Struktur in die in (Holling 2001)
...
\bibitem{Janich 2006} Peter Janich (2006). Was ist Information?
  Frankfurt/Main.
